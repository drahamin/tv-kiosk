...
class RemoteControlTests(unittest.TestCase):
    def test_samsung_remote_controls_navigation_and_zoom(self):
        ...
        self.assertIn("timeout 5 cec-ctl", script)
        self.assertIn("Physical Address", script)
        self.assertIn('[ "$device_count" -eq 1 ]', script)
    ...
    def test_labwc_hides_and_warps_cursor(self):
        ...
        self.assertEqual(config.count('action name="ToggleFullscreen"'), 2)
        self.assertIn('identifier="*RahaminPrimary*" serverDecoration="no"', config)
        self.assertIn('identifier="*BaiamonteSecondary*" serverDecoration="no"', config)
