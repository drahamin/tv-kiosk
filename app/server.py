...
def validate_config(config):
    ...
    if secondary_zoom_percent not in ZOOM_LEVELS:
        raise ValueError("The Baiamonte second-display zoom must be one of the available levels")
    return {
        "title": title,
        "listen_port": port,
        ...
        "secondary_display_url": secondary_display_url,
        "secondary_zoom_percent": secondary_zoom_percent,
...
def system_snapshot():
    ...
    ethernet_device = network_device("ethernet", "eth0")
    ethernet_connection = run_command(["nmcli", "-g", "GENERAL.CONNECTION", "device", "show", ethernet_device])
    ipv4_addresses = [address for address in run_command(["hostname", "-I"]).split() if ":" not in address]
    ...
    audio = run_command(["wpctl", "get-volume", "@DEFAULT_AUDIO_SINK@"]) or "Unavailable"
    ...
    return {
        ...
        "address": ", ".join(ipv4_addresses) or "Unavailable",
        "ethernet": f"{ethernet_device} · {ethernet_connection}" if ethernet_connection and ethernet_connection != "--" else "Not connected",
        ...
        "profile": "Pi Zero · Baiamonte single page" if HARDWARE_PROFILE == "zero" else "Multi-page Raspberry Pi",
        "hdmi_outputs": ", ".join(hdmi_connectors) or "Unavailable",
...
def network_device(connection_type, fallback):
    for line in run_command(["nmcli", "-t", "-f", "DEVICE,TYPE", "device", "status"]).splitlines():
...
def admin_page(config, session, message="", error=False, snapshot=None):
    ...
    page_rows = ""
    page_limit = 1 if HARDWARE_PROFILE == "zero" else MAX_PAGES
    for index, page in enumerate(config["pages"][:page_limit], 1):
    ...
    secondary_zoom_options = "".join(f'<option value="{level}"{" selected" if config["secondary_zoom_percent"] == level else ""}>{level}%</option>' for level in ZOOM_LEVELS)
...
<h3>Ethernet</h3><div class="grid"><label class="check"><input type="checkbox" name="ethernet_enabled"{checked(net['ethernet_enabled'])}>Ethernet connection enabled</label><label>IPv4 mode<select name="ethernet_ipv4_mode"><option value="auto"{selected(net['ethernet_ipv4_mode'],'auto')}>Automatic (DHCP)</option><option value="manual"{selected(net['ethernet_ipv4_mode'],'manual')}>Static</option></select></label><label>IPv4 address / prefix<input name="ethernet_ipv4_address" value="{html.escape(net['ethernet_ipv4_address'], quote=True)}"></label><label>IPv4 gateway<input name="ethernet_ipv4_gateway" value="{html.escape(net['ethernet_ipv4_gateway'], quote=True)}"></label><label>IPv4 DNS<input name="ethernet_ipv4_dns" value="{html.escape(net['ethernet_ipv4_dns'], quote=True)}"></label><label>IPv6 mode<select name="ethernet_ipv6_mode"><option value="auto"{selected(net['ethernet_ipv6_mode'],'auto')}>Automatic</option><option value="manual"{selected(net['ethernet_ipv6_mode'],'manual')}>Static</option><option value="disabled"{selected(net['ethernet_ipv6_mode'],'disabled')}>Disabled</option></select></label><label>IPv6 address / prefix<input name="ethernet_ipv6_address" value="{html.escape(net['ethernet_ipv6_address'], quote=True)}"></label><label>IPv6 gateway<input name="ethernet_ipv6_gateway" value="{html.escape(net['ethernet_ipv6_gateway'], quote=True)}"></label><label>IPv6 DNS<input name="ethernet_ipv6_dns" value="{html.escape(net['ethernet_ipv6_dns'], quote=True)}"></label></div><p class="flash error"><strong>Network changes can disconnect this browser.</strong> Verify the SSID, password, static address, gateway, and DNS before applying. The saved Wi-Fi password is never displayed.</p><div class="actions"><button class="danger" type="submit">Apply network configuration</button></div></form>'''
    ...
    remote_help = "Left/Right change pages; Up/Down zoom only the active ADS-B, AIS, or weather map; OK activates controls; Play/Pause pauses or resumes rotation; Fast-forward/Rewind zoom in/out. Samsung reserves the Volume and Channel rockers for television functions."
    second_display = ""
    if KIOSK_VARIANT != "baiamonte" and HARDWARE_PROFILE != "zero":
        second_display = f'''<section class="card"><div class="card-head"><span class="eyebrow">SECOND HDMI • PI 4 / PI 5</span><h2>Baiamonte display on HDMI 2</h2></div><div class="body grid"><label class="check"><input type="checkbox" name="secondary_display_enabled"{' checked' if config['secondary_display_enabled'] else ''}>Enable the independent Baiamonte TV dashboard</label><label class="wide">Baiamonte dashboard URL<input type="url" name="secondary_display_url" value="{html.escape(config['secondary_display_url'], quote=True)}" required></label><label>Second-display zoom<select name="secondary_zoom_percent">{secondary_zoom_options}</select></label><p class="note wide">Available only when a Raspberry Pi 4 or 5 detects two connected HDMI displays. HDMI 1 keeps the Rahamin five-page kiosk; HDMI 2 shows Baiamonte continuously. Pi 3 and Pi Zero ignore this setting safely.</p></div></section>'''
...
<form method="post" action="/admin/settings"><input type="hidden" name="csrf" value="{session['csrf']}"><section class="card"><div class="card-head"><span class="eyebrow">PLAYLIST</span><h2>{playlist_title}</h2></div><div class="body">{page_rows}<p class="note">{playlist_note}</p></div></section>
<section class="card"><div class="card-head"><span class="eyebrow">DISPLAY</span><h2>Rahamin Kiosk appearance, audio, and timing</h2></div><div class="body grid"><label>Display title<input name="title" value="{html.escape(config['title'], quote=True)}" required></label>{timing_fields}<label>Page zoom<select name="zoom_percent">{zoom_options}</select></label><label>HDMI audio volume<input type="number" min="0" max="100" name="audio_volume" value="{config['audio_volume']}" required></label><label class="check"><input type="checkbox" name="audio_enabled"{' checked' if config['audio_enabled'] else ''}>Enable audio through the TV</label><label>Theme<select name="theme"><option value="rahamin"{' selected' if config['theme']=='rahamin' else ''}>Rahamin signature</option><option value="midnight"{' selected' if config['theme']=='midnight' else ''}>Midnight aviation</option><option value="coastal"{' selected' if config['theme']=='coastal' else ''}>Miami coastal</option></select></label><label>Screen background<input type="color" name="background" value="{config['background']}"></label><label class="check"><input type="checkbox" name="show_status"{show_checked}>Show page name on pointer movement</label><label class="check"><input type="checkbox" name="setup_complete"{setup_checked}>{setup_label}</label><p class="note wide">HDMI audio uses the Pi's existing hardware output and PipeWire service; no extra media server or transcoding is added. <strong>Samsung remote (Anynet+):</strong> {remote_help}</p></div></section>
{second_display}
...
def active_pages(config):
    pages = [page for page in config["pages"] if page["enabled"]]
    return pages[:1] if HARDWARE_PROFILE == "zero" else pages
