...
BAIAMONTE_TV_URL=${BAIAMONTE_TV_URL:-http://rahamin-adsb.local:8998/tv}
...
if [ -n "$WIFI_PSK" ]; then
  for wifi_spec in "$WIFI_PRIMARY_SSID:20" "$WIFI_SECONDARY_SSID:10"; do
    ...
    nmcli connection modify "$wifi_name" connection.autoconnect yes connection.autoconnect-priority "$wifi_priority" ipv4.method auto ipv6.method auto wifi-sec.key-mgmt wpa-psk wifi-sec.psk "$WIFI_PSK"
  done
fi
